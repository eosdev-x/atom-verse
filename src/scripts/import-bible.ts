...
function validateVerse(verse: BibleVerse): string | null {
...
async function validateBibleData(verses: BibleVerse[]): Promise<ValidationError[]> {
...
async function processBible(): Promise<void> {
  try {
    ...
    const bookStats = new Map<string, { chapters: Set<number>, verses: number }>();
    ...
    // Create processed output directory in public folder
